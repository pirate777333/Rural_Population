...
df=pd.read_csv('D:/rektorov_rad/programirano/Ruralno/2d80dc86-7385-4b25-a67b-1e429a31ae27_Data.csv')
df=df.iloc[:-5,2:-1]
...
fig=px.bar(df.iloc[0:10],y='2019',x='Country Name',color='Country Name',
           text='2019',template='plotly_dark',title='(Highest) Population in rural areas')
fig.show()

df=df.sort_values(by='2019', ascending=True)
print(df.iloc[10:20,:])

fig=px.bar(df.iloc[10:20],y='2019',x='Country Name',color='Country Name',
           text='2019',template='plotly_dark',title='(Lowest) Population in rural areas')
fig.show()
...
fig = px.choropleth(df,                            # Input Dataframe
                     ...
                     range_color=[0,900000000]              # select range of dataset
                     )        
fig.show()
